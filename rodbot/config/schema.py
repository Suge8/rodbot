...
class AgentDefaults(Base):
    ...
    workspace: str = "~/.rodbot/workspace"
...
class Config(BaseSettings):
    ...
    def _match_provider(
        ...
        Priority: prefix match → keyword match → fallback.
        """
        ...
        # Priority 1: "provider/model" prefix → exact provider name match
        if "/" in model_str:
            prefix = model_str.split("/", 1)[0].lower().replace("-", "_")
            for spec in PROVIDERS:
                if spec.name == prefix:
                    p = getattr(self.providers, spec.name, None)
                    if p and (spec.is_oauth or spec.is_direct or p.api_key):
                        return p, spec.name
    ...
    model_config = ConfigDict(env_prefix="rodbot_", env_nested_delimiter="__")
